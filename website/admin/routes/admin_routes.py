...
from logic.admin_logic import create_service, delete_service, update_service
...
def admin_services():
    ...
    return render_template('admin/admin-services.html', services=services, current_page=current_page)
...
def admin_create_service():
    if request.method == "POST":
        ...
        if uploaded_file:
            ...
            image_path = os.path.join('static', 'images', 'client', 'services', service_name, image_filename)
...
def admin_edit_service(service_id):
    ...
    if request.method == 'POST':
        ...
        if uploaded_file:
            # Компонование пути к файлу
            image_filename = secure_filename(uploaded_file.filename)
            new_image_path = os.path.join('static', 'images', 'main-page', service_name, image_filename)
    ...
    return render_template('admin/admin-service-edit.html', edit_mode=edit_mode, service_data=service_data)