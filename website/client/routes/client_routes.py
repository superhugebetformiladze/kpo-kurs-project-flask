...
from logic.service_logic import read_all_services_except, read_services, read_service_by_id
...
def service_detail(service_id):
    ...
    all_services = read_all_services_except(service_id)
    if len(all_services) >= 3:
        random_services = random.sample(all_services, 3)
    else:
        random_services = all_services
    return render_template('client/service_detail.html', service=service, services=random_services)
